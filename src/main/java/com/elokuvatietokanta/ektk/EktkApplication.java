...
public class EktkApplication {
    ...
    public CommandLineRunner ektkDemo(CategoryRepository categoryRepository, MovieRepository movieRepository) {
        return (args) -> {
            ...
            movieRepository.save(new Movie("The Shawshank Redemption", 1994, 9.2f, categoryRepository.findByName("Draama").get(0)));
            movieRepository.save(new Movie("The Godfather", 1972, 9.2f, categoryRepository.findByName("Draama").get(0)));
            movieRepository.save(new Movie("The Godfather: Part II", 1974, 9.2f, categoryRepository.findByName("Draama").get(0)));
            movieRepository.save(new Movie("The Dark Knight", 2008, 9.2f, categoryRepository.findByName("Draama").get(0)));
            movieRepository.save(new Movie("12 Angry Men", 1957, 9.2f, categoryRepository.findByName("Draama").get(0)));
            movieRepository.save(new Movie("Schindler's List", 1993, 9.2f, categoryRepository.findByName("Draama").get(0)));
            movieRepository.save(new Movie("Pulp Fiction", 1994, 9.2f, categoryRepository.findByName("Draama").get(0)));
            movieRepository.save(new Movie("The Lord of the Rings: The Return of the King", 2003, 9.2f, categoryRepository.findByName("Draama").get(0)));
